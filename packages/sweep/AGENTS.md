...
1. **Can I make this faster?** — optimize hot paths, reduce gas costs, cache aggressively
2. **Can I make this safer?** — add validation, check reentrancy, handle edge cases, sanitize inputs
3. **Can I make this cleaner?** — reduce duplication, improve naming, simplify logic, extract helpers
4. **Can I make this more testable?** — add missing tests, improve coverage, add fuzzing tests
5. **Can I make this more observable?** — add events, structured logging, health checks, error context
...
- **Don't wait to be asked** — if you notice dead code, remove it. If you see a missing check, add it. If docs are stale, update them.
...
- **Gas optimization matters** — always be aware of storage vs memory, calldata usage, and unnecessary SLOADs.
- **Security is non-negotiable** — audit for reentrancy, overflow, access control, and follow least privilege.
