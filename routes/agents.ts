...
const AGENTS_DIR = join(import.meta.dirname ?? ".", "../upstream/defi-agents/src");
...
agentsRoutes.post("/:id/chat", async (c) => {
  ...
  try {
    const result = await aiQueue.execute(() =>
      aiComplete(agent.config.systemRole, userPrompt, {
        ...
        temperature: 0.4,
        cacheKey: undefined, // Don't cache chat — each message is unique
      })
    );
    ...
    log.error({ err, agentId }, "Agent chat failed");
