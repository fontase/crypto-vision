...
class NotifyConfig:
    ...
    def from_env(cls) -> "NotifyConfig":
        ...
        - XTOOLS_NOTIFY_CONSOLE: "true" or "false"
        - XTOOLS_EMAIL_*: Email configuration
        - XTOOLS_WEBHOOK_URL: Webhook URL
        - XTOOLS_TELEGRAM_*: Telegram configuration
        """
        import os
        
        config = cls(
            console=os.environ.get("XTOOLS_NOTIFY_CONSOLE", "true").lower() == "true",
        )
        
        # Email config
        if os.environ.get("XTOOLS_EMAIL_SMTP_HOST"):
            try:
        ...
        # Webhook
        webhook_url = os.environ.get("XTOOLS_WEBHOOK_URL")
        if webhook_url:
            config.webhook_url = webhook_url
        
        # Telegram
        if os.environ.get("XTOOLS_TELEGRAM_BOT_TOKEN"):
            try:
...
class NotificationManager:
    ...
    async def test_all(self) -> Dict[str, bool]:
        ...
        return await self.notify(
            "test",
            "This is a test notification from XTools",
