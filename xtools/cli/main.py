"""
XTools CLI - Main entry point.

The primary command-line interface for XTools.
"""
...
from xtools.cli.commands import scrape, follow, unfollow, engage, monitor, ai
...
def load_config(config_path: str) -> dict[str, Any]:
    ...
    if not path.exists():
        ...
        for default_path in ["config.yaml", "~/.xtools/config.yaml"]:
...
@click.version_option(version="0.1.0", prog_name="xtools")
@click.pass_context
def cli(ctx: click.Context, config: str, verbose: bool, quiet: bool) -> None:
    ...
    \b
        xtools --help              Show this help
        xtools scrape --help       Scraping commands
        xtools ai reply "Hello"    Generate AI reply
    """
...
def status(ctx: click.Context) -> None:
    ...
    console.print("\n[bold blue]XTools Status[/bold blue]\n")
...
def init(ctx: click.Context, provider: str) -> None:
    """Initialize XTools configuration."""
    import yaml
    
    console = ctx.obj["console"]
    
    console.print("\n[bold blue]XTools Initialization[/bold blue]\n")
    ...
    console.print("  2. Run: xtools status")
...
try:
    ...
    app = typer.Typer(
        name="xtools",
        help="XTools - X/Twitter Automation Toolkit",
